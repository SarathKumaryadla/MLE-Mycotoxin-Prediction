...
def train_random_forest(X_train, y_train, n_estimators=200, max_depth=8, min_samples_split=5, min_samples_leaf=5):
    """Trains an optimized Random Forest Regressor with hyperparameter tuning."""
    model = RandomForestRegressor(
        n_estimators=n_estimators,  # Increased number of trees for better stability
        max_depth=max_depth,  # Reduced depth to prevent overfitting
        min_samples_split=min_samples_split,  # Ensures splits have enough samples
        min_samples_leaf=min_samples_leaf,  # Prevents overly small leaves
        random_state=42
    )
    model.fit(X_train, y_train)
    return model
